...
if ( !function_exists( 'eve_gtm_script' ) ) :
	/**
	 * GTM Code
	 */
	function eve_gtm_script()
	{ 
...
if ( !function_exists( 'eve_gtm_noscript' ) ) :
	/**
	 * GTM Code
	 */
	function eve_gtm_noscript()
	{
    $gtm_ID = get_option('eve_gtm_code');
    if( empty($gtm_ID) ){ return; }
...
if ( ! function_exists( 'eve_about_comment_in_html' ) ) :
	/**
	 * About in comments
	 */
	function eve_about_comment_in_html()
	{
		if( get_option('eve_environment_details') == 0 )
			return;
		
		if( get_option('eve_html_compression') == 1 ){
			$ignore_comment = '<!--wp-html-compression no compression-->';
		}else{
			$ignore_comment = '';
		}
